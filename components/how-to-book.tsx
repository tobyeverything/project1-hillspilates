...
const steps = [
  {
    ...
    description:
      "Explore our timetable and find a class that fits your schedule and level — from beginner mat work to advanced reformer sessions.",
    icon: Search,
  },
  {
    number: "02",
    ...
    description:
      "Tap the button below to message us directly. Let us know which class you're interested in and we'll confirm availability.",
    icon: MessageCircle,
  },
  {
    number: "03",
    ...
    description:
      "Once we've chatted, we'll lock in your spot. You'll receive a confirmation with everything you need to know before your session.",
    icon: CalendarCheck,
  },
]
...
export function HowToBook() {
  ...
  useEffect(() => {
    ...
    // 0–40%: figure scales in, arm stays down, question mark hidden
    tl.fromTo(
      figureRef.current,
      { scale: 0.1, opacity: 0 },
    ...
    // 40–100%: arm raises to scratch head, question mark appears
    tl.fromTo(
      leftArm,
    ...
    <section ref={sectionRef} id="book" className="w-full bg-muted">
      <div className="px-6 py-20 md:px-10 md:py-28 lg:px-16 lg:py-32">
        ...
        <div ref={ctaRef} className="mt-10 flex justify-center opacity-0">
          <Button
            size="lg"
            className="gap-3 bg-[#25D366] px-8 py-6 text-lg font-semibold text-white hover:bg-[#1ebe57] md:text-xl"
            asChild
          >
            <a
